...
class sun:
    ...
    def __init__(self, lat=51.6080, long=-1.2448):  # default Didcot
        self.lat = lat
        self.long = long
    ...
    def sunrise(self, when):
        ...
        self.__calc()
        return self.__utc_time_from_decimal_day(self.sunrise_t)

    def sunset(self, when):
        self.__preptime(when)
        self.__calc()
        return self.__utc_time_from_decimal_day(self.sunset_t)

    def solarnoon(self, when):
        self.__preptime(when)
        self.__calc()
        return self.__utc_time_from_decimal_day(self.solarnoon_t)

    def __local_time_from_decimal_day(self, day):
        t = sun.__timefromdecimalday(day)
        tzdelta = timedelta(seconds=int(self.timezone*3600))
        t = (datetime.combine(date.today(), t) +
             tzdelta).time()
        return t

    def __utc_time_from_decimal_day(self, day):
        return sun.__timefromdecimalday(day)
    ...
    def __timefromdecimalday(day):
        ...
        """
        assert day >= 0.0
        assert day <= 1.0, "Decimal day is actually %f, must be < 1.0" % day
        hours = 24.0*day
        h = int(hours)
        minutes = (hours-h)*60
        m = int(minutes)
